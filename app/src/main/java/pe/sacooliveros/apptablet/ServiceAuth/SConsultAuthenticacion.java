package pe.sacooliveros.apptablet.ServiceAuth;
...
public class SConsultAuthenticacion extends AsyncTask<String, Integer, ArrayList> {
    ...
    public SConsultAuthenticacion(final String URL) {
        this.url = URL;
    }
    ...
    protected ArrayList doInBackground(String... strings) {
        ...
        String rString = authservice.respuestaFromWebService(url, 8080, "", strings);
        Log.e("RESPONSE SACO", rString);

        ArrayList<JSONObject> resValues = new ArrayList<>();

        if (ServiceAuthentication.isValidResponse(rString)) {
            try {
